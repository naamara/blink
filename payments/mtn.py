...
class Mtn():
	...
	def __init__(self):
		...
		self.OpCoID = 'UG'
		self.SenderID = 'MTN'
		self.VendorCode='%s' % settings.MTN_VENDOR_CODE
		self.TypeCode='GSM'
	...
	def DepositMoney(self, amount, number, transactionid, ref_text=""):
		...
		url = '%s/ThirdPartyServiceUMMImpl/UMMServiceService/DepositMobileMoney/v17' % settings.MTN_SDP_URL
		headers = {
	...
	def SendNotification(self):
		...
		deposit Mobile Money into users phonenumber
		'''
		url = '%s/SendNotification' % settings.MTN_SDP_URL
	...
	def CheckNumber(self, number="789999550", OperationType="ProfileDetails"):
		...
		#"VendorCode": "%s" % settings.MTN_VENDOR_CODE,
		"CommonComponents": {
		"MSISDNNum": "%s" % number,
		...
		import requests
		try:
			url = '%s/1/generic/processcustomer' % settings.MTN_SDP_URL
	...
	def validateAcountHolder(self, number="256789945550"):
		...
		url = '%s:8323/mom/mt/validateaccountholder' % settings.MTN_SDP
